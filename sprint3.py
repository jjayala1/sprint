...
class Sprint():
    ...
    def main(self):
        ...
        if self.data != '':
            ...
            try:
                ...
                print(f"#There is no data about likes and comments {self.file}")
    ...
    def insert_tarea(self):
        ...
        sql2 = f"UPDATE posts set update_date='{curtime}', num_comments='{self.num_comments}', num_likes='{self.num_likes}' WHERE day='{self.day}' AND (owner='{self.owner}' OR link='{self.link}');"
        print(sql2)
        return 0
...
if __name__ == '__main__':
    ...
    def process_html():
        for file_post in sorted(os.listdir("./posts")):
            #if file_post[-4:] == 'html':
            #if file_post[3:5] == '30':
            if 'Tom' in file_post and file_post.endswith('html'):
                #print('------------------------------------------------------------------------------------------------------------------------')
                #print(file_post)
                origin = file_post
                dest = file_post + '.proc'
                num_day = file_post[3:5]
